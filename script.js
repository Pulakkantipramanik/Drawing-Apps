...
undoBtn = 
    document.querySelector(".undo"),  // Undo button
...
let prevMouseX, prevMouseY, snapshot,
    ...
    brushWidth = 5,
    selectedColor = "#000",
    undoStack = [];  // Stack to store undo states
...
const drawArrow = (e) => {
    ...
    // Draw arrowhead
    ctx.beginPath();
    ctx.moveTo(e.offsetX - headLength * Math.cos(angle - Math.PI / 6), 
    e.offsetY - headLength * Math.sin(angle - Math.PI / 6));
    ctx.lineTo(e.offsetX, e.offsetY);
    ctx.lineTo(e.offsetX - headLength * Math.cos(angle + Math.PI / 6),
    e.offsetY - headLength * Math.sin(angle + Math.PI / 6));
    ctx.closePath();
    ctx.fill();
}
...
cleatCanvas.addEventListener("click", () => {
    ...
    setCanvasBackground();
    undoStack = [];  // Clear the undo stack when the canvas is cleared
});
